...
def create_msg(msg_info, msg_info_printed, params):
    ...
    for key, values in msg_info.items():
        for val in values:
            ...
            if re.search("ref_provider|patient_|guardian|comments|project_id|record_id|reason_other|note_|_note", key, re.IGNORECASE):
                NTE[0]="NTE"
                NTE[1] = str(nte_count)
                length = len(key)
    ...
    for key, values in msg_info.items():
        ...
        if msg_info_printed[key] == "0":
            for val in values:
                ...
                msg_info_printed[key] = "1"
    # --------------------------------------------------
    #add extra NTE at the end of all NTE for referral team to add notes

    key = "Central_transcription_notes"
    val = ""
    NTE[0] = "NTE"
    NTE[1] = str(nte_count)
    length = len(key)
    ...
    for encode_file in msg_info["encode_files"]:
       file_stat = os.stat(encode_file) 
       file_size += file_stat.st_size/(1024*1024) 
       logger.debug(f"file size is {file_size} MB" )  
    if file_size > float(max_file_size):
        return "large_file"
...
def query_db(config): 
    ...
    params = {}  
    params["max_file_size"] =config["max_file_size"] 
    params["processing_id"] = config["processing_id"] 
    ...
    try:
        ...
        query1 = """SELECT distinct project_id, record, event_id
                ...
                WHERE
                # condition "A" "checks if valid project-----
                rd1.project_id IN ({})
                AND
                # condition "B" "checks if referral is complete"         
                rd1.record IN 
                (SELECT rd2.record FROM redcap_data rd2
                 WHERE rd2.field_name = 'referral_complete' AND rd2.value = '2'
                AND rd2.project_id = rd1.project_id)
                AND
                # condition "C" "checks if this record has large attachments"
                ((rd1.record IN
                  (SELECT rd3.record FROM redcap_data rd3
                   WHERE rd3.field_name = 'extract_complete' AND rd3.value < 1
                   AND rd3.project_id = rd1.project_id))
                   OR
                    ((rd1.record NOT IN
                     (SELECT rd4.record FROM redcap_data rd4
                      WHERE rd4.field_name = 'extract_complete'
                      AND rd4.project_id = rd1.project_id))
                     ))
                AND
                #condition "D" "checks if record was already processed into Epic"
                ((rd1.record IN
                (SELECT rd5.record FROM redcap_data rd5
                 WHERE rd5.field_name = 'status_complete' AND rd5.value <> 2
                AND rd5.project_id = rd1.project_id))
                OR
                ((rd1.record NOT IN
                (SELECT rd6.record FROM redcap_data rd6
                WHERE rd6.field_name = 'status_complete'
                AND rd6.project_id = rd1.project_id))
                ))""".format(allowed_projects) 
        ...
        for project_id, record, event_id in rows:
            ...
            if duplicate_check == "yes":
                ...
                for field_name,value in rec_rows:
                    dup_info[field_name] = value
                patient_first_name = dup_info["patient_first_name" ] 
                patient_last_name = dup_info["patient_last_name" ] 
                patient_dob = dup_info["patient_dob" ] 
                patient_gender = dup_info["patient_gender" ] 
                guardian_phone = dup_info["guardian_phone" ] 
                guardian_phone = guardian_phone.replace('(','').replace(')','').replace('-','')   
                dup_query =f"""
Select count(record) From (
SELECT    
rd.project_id,rd.event_id,rd.record,
Max(Case When rd.field_name='patient_first_name' then rd.value else null end) As patient_first_name,
Max(Case When rd.field_name='patient_last_name' then rd.value else null end) As patient_last_name,
Max(Case When rd.field_name='patient_dob' then rd.value else null end) As patient_dob,
Max(Case When rd.field_name='patient_gender' then rd.value else null end) As patient_gender,
Max(Case When rd.field_name='guardian_phone' then rd.value else null end) As guardian_phone,
Max(Case When rd.field_name='date_referred' then rd.value else null end) As date_referred,
Max(Case When rd.field_name='status_complete' then rd.value else null end) As status_complete

FROM redcap_data rd 
Where rd.project_id= {project_id} 
Group By  rd.project_id,rd.event_id,rd.record
) q
Where q.project_id={project_id}  and q.patient_first_name='{patient_first_name} ' and q.patient_last_name='{patient_last_name} ' and q.patient_dob='patient_dob' 
and q.patient_gender='{patient_gender} '  and q.status_complete=2
AND REPLACE(REPLACE(REPLACE(REPLACE(q.guardian_phone, '-', ''), ')', ''), '(', ''), ' ', '') 
= '{guardian_phone} '
AND q.date_referred BETWEEN CURDATE() - INTERVAL 30 DAY AND CURDATE()
                """ 
                logger.debug(f"dup querry:{dup_query}  " ) 
                dup_cursor = cnx.cursor() 
                dup_cursor.execute(dup_query)   
                dups = dup_cursor.fetchone()  
                del dup_cursor
                dup_count = dups[0] 
                if dup_count >= 2:
                    ...
                    if stnotes_count == 0:
                        ...
                        VALUES ({},{},{},'status_notes', "This is inserted by interface on {} and found that duplicate record exists" )""".format(project_id, event_id, record, dbdate)  
            ...
            result = create_msg(info, info_printed,params )
            logger.debug(f"file size is {result}" )   
            ...
            if result == "ok":
            ...
            elif result == "large_file":
                update_sql = """ update redcap_data
                                     set value = 2
                                     where project_id = {} 
                                     and record = {}
                                     and field_name = "extract_complete"
                                     """.format(project_id, record  )
                logger.debug(f"executed update query for large_file:{update_sql}"  ) 
                update_cursor = cnx.cursor()
                affected_row = update_cursor.execute(update_sql) 
                logger.debug(f"rows affected {affected_row}" ) 
                cnx.commit() 
                ...
                if stnotes_count == 0:
                    ...
                    ins_stmt = """insert into redcap_data
                    (project_id, event_id,record, field_name, value) 
                    VALUES ({},{},{},'status_notes', "Interface skipped this record due to maximum attachment size exceeded {} MB at {}" )""".format(project_id, event_id, record, max_file_size,dbdate)  
                    logger.debug(f"insert stmt {ins_stmt}" ) 
                    ins_cur = cnx.cursor() 
                    ins_cur.execute(ins_stmt) 
                    cnx.commit() 
